...
#ifdef ROSE
cl::opt<bool> ExtractDefaultValues(
    "extract-default-values", cl::init(false),
    cl::desc("Extract default values from the program (default=false)"),
    cl::cat(DebugCat));
// string opt "extract-default-values-filepath"
cl::opt<std::string> ExtractDefaultValuesFilePath(
    "extract-default-values-filepath", cl::init(""),
    cl::desc("File path to save the extracted default values (default=empty)"),
    cl::cat(DebugCat));
// string opt "extract-default-values-function-name"
cl::opt<std::string> ExtractDefaultValuesFunctionName(
    "extract-default-values-function-name", cl::init(""),
    cl::desc("Function name to extract default values from (default=empty)"),
    cl::cat(DebugCat));
// unsigned opt "extract-default-values-line-no"
cl::opt<unsigned> ExtractDefaultValuesLineNo(
    "extract-default-values-line-no", cl::init(0),
    cl::desc("Line number to extract default values from (default=0)"),
    cl::cat(DebugCat));
// string opt "state-variables-filepath"
cl::opt<std::string> StateVariablesFilePath(
    "state-variables-filepath", cl::init(""),
    cl::desc("File path to read state variables from (default=empty)"),
    cl::cat(DebugCat));
// string opt "initial-sequences-filepath"
cl::opt<std::string> InitialSequencesFilePath(
  "initial-sequences-filepath", cl::init(""),
  cl::desc("File path to read initial sequences from (default=empty)"),
  cl::cat(DebugCat));
// unsigned opt "server-port"
cl::opt<unsigned> ServerPort(
    "server-port", cl::init(12345),
    cl::desc("Port number of the server to connect to (default=12345)"),
    cl::cat(DebugCat));

std::vector<std::vector<std::string>> _readTextFile(const std::string& filePath) {
...
std::vector<std::vector<std::string>> readStateVariables(const std::string& filePath) {
  return _readTextFile(filePath);
}

std::vector<std::vector<std::string>> readInitialSequences(const std::string& filePath) {
  std::ifstream file(filePath);
  if (!file.is_open()) {
      llvm::errs() << "Empty initial sequences.\n";
      return {};
  }
  file.close();
  return _readTextFile(filePath);
...
std::vector<ExecutionState*> selectStatesToBeUpdated(PTree *processTree) {
  return {};
  auto root = processTree->root;
  unsigned int MAX_NUM = 100;
  unsigned int MAX_LEAVES = 100;
  std::vector<PTreeNode *> leaves;
  std::queue<PTreeNode *> q;
  #ifdef ROSE_OPTION_SEARCHER
  q.push(root.first);
  #else
  q.push(root.getPointer());
  #endif
  while (!q.empty() && leaves.size() < MAX_LEAVES) {
    PTreeNode *node = q.front();
    q.pop();
    #ifdef ROSE_OPTION_SEARCHER
    if (node->left.first == nullptr && node->right.first == nullptr) {
      leaves.push_back(node);
    } else {
      if (node->left.first != nullptr) {
        q.push(node->left.first);
      }
      if (node->right.first != nullptr) {
        q.push(node->right.first);
      }
    }
    #else
    if (node->left.getPointer() == nullptr && node->right.getPointer() == nullptr) {
      leaves.push_back(node);
    } else {
      if (node->left.getPointer() != nullptr) {
        q.push(node->left.getPointer());
      }
      if (node->right.getPointer() != nullptr) {
        q.push(node->right.getPointer());
      }
    }
    #endif
  }
  if (!q.empty()) {
    ...
    for (unsigned int i = 0; i < MAX_NUM; i++) {
      #ifdef ROSE_OPTION_SEARCHER
      PTreeNode *node = root.first;
      while (node->left.first != nullptr || node->right.first != nullptr) {
        if (rand() % 2 == 0) {
          if (node->left.first != nullptr) {
            node = node->left.first;
          } else if (node->right.first != nullptr) {
          ...
          if (node->right.first != nullptr) {
            node = node->right.first;
          } else if (node->left.first != nullptr) {
            node = node->left.first;
          } else {
            break;
          }
      ...
      #else
      PTreeNode *node = root.getPointer();
      while (node->left.getPointer() != nullptr || node->right.getPointer() != nullptr) {
        if (rand() % 2 == 0) {
          if (node->left.getPointer() != nullptr) {
            node = node->left.getPointer();
          } else if (node->right.getPointer() != nullptr) {
            node = node->right.getPointer();
          } else {
            break;
          }
        } else {
          if (node->right.getPointer() != nullptr) {
            node = node->right.getPointer();
          } else if (node->left.getPointer() != nullptr) {
            node = node->left.getPointer();
          } else {
            break;
          }
  ...
  std::set<int> visited;
  for (PTreeNode *leaf : leaves) {
    int id = leaf->state->getID();
    if (visited.find(id) == visited.end()) {
      visited.insert(id);
      result.push_back(leaf->state);
    }
  }
  return result;
}

OptionVarMap globalOptionVarMap;
bool encounterSymbol = false;
bool updateOptions = false;
Solver* coreSolverPtr = nullptr;
bool forkNewOptionStates = false;
std::vector<void *> newOptionStates;
bool priorityStateSelected = false;
std::vector<std::string> concreteOptionsOfCurrentState;
std::vector<std::vector<std::string>> pendingNewSequences;
unsigned int pendingNewSequencesLatestIndex = 0;
bool solverSat = false;
bool solverSatEnabled = false;
std::map<std::string, std::pair<llvm::Value*, std::pair<uint64_t, size_t>>> globalVariables;
std::map<llvm::Value*, std::pair<uint64_t, size_t>> globalVariableAddresses;
llvm::Module *GlobalTheModule = nullptr;
std::set<std::vector<std::string>> visitedConcreteArgs;
std::map<const llvm::GlobalValue*, ref<ConstantExpr>> Executor::getGlobalAddresses() const {
  return globalAddresses;
}
int serverSocket;
llvm::Instruction *clonePosition;
#endif
} // namespace klee
...
static void clearGlobalVariables() {
  ...
  globalVariableAddresses.clear();
  GlobalTheModule = nullptr;
}
std::string resolveManagedSymbol(const std::string &name) {
  std::vector<std::string> possibleNames;
  for (const auto &pair : globalVariables) {
    if (pair.first.size() >= name.size() &&
        pair.first.compare(pair.first.size() - name.size(), name.size(), name) == 0) {
      possibleNames.push_back(pair.first);
    }
  }
  if (possibleNames.empty()) {
    llvm::errs() << "No global variable found with name: " << name << "\n";
    return name;
  }
  if (possibleNames.size() > 1) {
    llvm::errs() << "Multiple global variables found with name: " << name
                 << "\n";
    for (const auto &varName : possibleNames) {
      llvm::errs() << "Found: " << varName << "\n";
    }
    exit(EXIT_FAILURE);
  }
  return possibleNames[0];
}
void get_module_symbols(Executor *executor, llvm::Module *TheModule) {
  GlobalTheModule = TheModule;
  assert(TheModule);
  llvm::ValueSymbolTable &VST = TheModule->getValueSymbolTable();
  for (llvm::ValueSymbolTable::iterator i = VST.begin(); i != VST.end(); i++) {
    if (llvm::isa<llvm::Function>(i->getValue())) {
        continue;
    }
    if (llvm::isa<llvm::GlobalVariable>(i->getValue())) {
      llvm::GlobalVariable* gv = llvm::cast<llvm::GlobalVariable>(i->getValue());
      if (gv->isConstant()) {
          continue;
      }
      std::string varName = gv->getName().str();
      assert (globalVariables.find(varName) == globalVariables.end());
      llvm::Value* value = VST.lookup(varName);
      assert (value);
      auto globalAddresses = executor->getGlobalAddresses();
      assert (globalAddresses.find(gv) != globalAddresses.end());
      uint64_t address = executor->getGlobalAddresses()[gv].get()->getZExtValue();
      assert (gv->getType()->isPointerTy());
      size_t size = getTypeSize(gv->getType()->getPointerElementType());
      globalVariables[varName] = std::make_pair(value, std::make_pair(address, size));
      globalVariableAddresses[value] = std::make_pair(address, size);
    }
  }
  for (auto& [varName, info] : globalVariables) {
    llvm::errs() << "Global variable: " << varName << "\n";
    llvm::errs() << "Address: " << info.second.first << "\n";
    llvm::errs() << "Size: " << info.second.second << "\n";
  }
  return;
}
#endif
...
Executor::Executor(LLVMContext &ctx, const InterpreterOptions &opts,
  ...
  #ifdef ROSE
  klee::coreSolverPtr = coreSolver.get();
  #endif
...
static std::string instantiate_arg_python(std::string argname) {
...
static std::string instantiate_arg_client(std::string argname) {
  std::string result;
...
#ifdef ROSE_DEBUG
  llvm::errs() << "Argname sent to server: " << argname << "\n";
...
read_flagname_argname_from_file(std::string filename) {
  std::map<std::string, std::string> flagname_argname;
  return flagname_argname;
  assert(false);
  ...
  while (std::getline(file, line)) {
    ...
    std::istringstream iss(line);
    std::string flagname, argname;
    iss >> flagname >> argname;
    if (flagname.empty() || argname.empty()) {
      llvm::errs() << "Bad format in " << filename << " at line " << line_num
                   << ": " << line << "\n";
      exit(1);
    }
    flagname_argname[flagname] = argname;
  }
#ifdef ROSE_DEBUG
  llvm::errs() << "flagname_argname with size " << flagname_argname.size()
               << ":\n";
  for (auto it = flagname_argname.begin(); it != flagname_argname.end(); ++it) {
    llvm::errs() << it->first << " " << it->second << "\n";
  }
#endif
  return flagname_argname;
}
static std::map<std::string, std::string> flagname_argname =
    read_flagname_argname_from_file("TODO:");
static std::set<std::string> flagname_visited;
static ExecutionState *initialStatePtr = nullptr;
static Function *startF = nullptr;
static int g_argc = 0;
static char **g_argv = nullptr;
static char **g_envp = nullptr;
std::vector<std::string>
Executor::getConcreteArgs(const ExecutionState &state) {
  return state.getConcreteArgs();
}
void Executor::updateArgs(ExecutionState *state, std::vector<std::string> concreteArgs) {
#ifdef ROSE_DEBUG
  llvm::errs() << "before updateArgs:\n";
  dumpState(state);
#endif
  MemoryObject *argvMO = 0;
  unsigned NumPtrBytes = Context::get().getPointerWidth() / 8;
  unsigned argc = concreteArgs.size() + g_argc;
  assert(g_argc >= 1 && "g_argc should be at least 1");
  assert(g_argv != nullptr && "g_argv should not be nullptr");
  assert(g_argv[0] != nullptr && "g_argv[0] should not be nullptr");
  std::vector<ref<Expr>> arguments;

  KFunction *kf = kmodule->functionMap[startF];
  assert(kf);

  assert(startF->arg_size() <= 3);
  int envc;
  for (envc = 0; g_envp[envc]; ++envc)
    ;
  Function::arg_iterator ai = startF->arg_begin(), ae = startF->arg_end();
  bool noArgvEnvp = false;
  if (ai != ae) {
    arguments.push_back(ConstantExpr::alloc(argc, Expr::Int32));
    if (++ai != ae) {
      Instruction *first = &*(startF->begin()->begin());
  ...
  assert(arguments.size() == startF->arg_size() && "wrong number of arguments");
  for (unsigned i = 0, e = arguments.size(); i != e; ++i)
    bindArgument(kf, i, *state, arguments[i]);

  assert(argvMO && "argvMO not set");
  ObjectState *argvOS = bindObjectInState(*state, argvMO, false);

  for (unsigned int i = 0; i < argc + 1 + envc + 1 + 1; i++) {
    if (i == argc || i >= argc + 1 + envc) {
      ...
      if (i < argc) {
        if (i == 0) {
          s = g_argv[0];
        } else if (i <= concreteArgs.size()) {
          assert(i - 1 < concreteArgs.size());
          s = concreteArgs[i - 1].c_str();
        } else {
          s = g_argv[i - concreteArgs.size()];
        }
      } else {
        s = g_envp[i - (argc + 1)];
      }
      int j, len = strlen(s);
      MemoryObject *arg =
          memory->allocate(len + 1, /*isLocal=*/false, /*isGlobal=*/true, state,
      ...
      #ifdef ROSE_DEBUG
      if (i < argc) {
        llvm::errs() << "Allocated memory for arg " << i << " at " << arg->address << " with content " << s << "\n";
      }
      #endif
    }
  }

  state->setConcreteArgs(concreteArgs);
#ifdef ROSE_DEBUG
  llvm::errs() << "after updateArgs:\n";
  dumpState(state);
  llvm::errs() << "updateArgs done\n";
#endif
  return;
}

uint64_t Executor::getLocalAddress(ExecutionState &state, std::string function_name, std::string var_name){
  uint64_t address = 0;
  StackFrame &sf = state.stack.back();
  if (sf.localAddresses.find(var_name) != sf.localAddresses.end()) {
    uint64_t localAddress = sf.localAddresses[var_name];
    address = localAddress;
  }
  return address;
}

llvm::DIType* Executor::getLocalDIType(ExecutionState &state, std::string function_name, std::string var_name){
  llvm::DIType *Ty = nullptr;
  StackFrame &sf = state.stack.back();
  if (sf.localDITypes.find(var_name) != sf.localDITypes.end()) {
    llvm::DIType *localDIType = sf.localDITypes[var_name];
    Ty = localDIType;
  }
  else {
    auto *mainFunc = this->kmodule->module->getFunction(function_name);
    assert(mainFunc != nullptr && "main function should not be null");
    bool found = false;
    for (auto &BB : *mainFunc) {
      for (auto &I : BB) {
        if (auto *ci = dyn_cast<CallInst>(&I)) {
          if (ci->getCalledFunction() && ci->getCalledFunction()->getName() == "llvm.dbg.declare") {
            llvm::errs() << "Found llvm.dbg.declare: " << *ci << "\n";
            if (auto *Arg1 = dyn_cast<MetadataAsValue>(ci->getArgOperand(1))) {
              if (auto *DILV = dyn_cast<DILocalVariable>(Arg1->getMetadata())) {
                auto name = DILV->getName();
                if (name == var_name) {
                  Ty = DILV->getType();
                  StackFrame &sf = state.stack.back();
                  sf.localDITypes[var_name] = Ty;
                  found = true;
                  break;
                }
              }
...
#ifdef ROSE_DEBUG
static std::string getDerivedTypeName(llvm::DIType *Ty) {
  DIDerivedType *DTy = nullptr;
  DICompositeType *CTy = nullptr;
  DIType *BTy = Ty;
  std::vector<std::string> pointer_array_vector;
  unsigned MAX_DEPTH = 10;
  unsigned depth = 0;
  while (true) {
    depth++;
    if (depth > MAX_DEPTH) {
      llvm::errs() << "Exceeded maximum depth of " << MAX_DEPTH
                   << " while processing type: " << BTy->getName() << "\n";
      exit(1);
    }
    DTy = dyn_cast<DIDerivedType>(BTy);
    if (!DTy) {
      CTy = dyn_cast<DICompositeType>(BTy);
      if (!CTy) {
        break;
      }
    }
    if (DTy) {
      if (DTy->getTag() == llvm::dwarf::DW_TAG_pointer_type) {
        llvm::errs() << "Pointer Type: " << DTy->getName() << "\n";
        pointer_array_vector.push_back("*");
      } else {
        llvm::errs() << "Unhandled Derived Type: " << DTy->getName() << "\n";
      }
      BTy = DTy->getBaseType();
    }
    if (CTy) {
      if (CTy->getTag() == llvm::dwarf::DW_TAG_array_type) {
        llvm::errs() << "Array Type: " << CTy->getName() << "\n";
        pointer_array_vector.push_back("[]");
      } else if (CTy->getTag() == llvm::dwarf::DW_TAG_structure_type ||
                 CTy->getTag() == llvm::dwarf::DW_TAG_class_type) {
        break;
      } else {
        llvm::errs() << "Unhandled Composite Type: " << CTy->getName() << "\n";
      }
      BTy = CTy->getBaseType();
    }
    DTy = nullptr;
    CTy = nullptr;
    assert(BTy != nullptr && "BTy should not be null");
  }
  auto type_name = BTy->getName();
  if (type_name.empty()) {
    type_name = "unknown_type";
  }
  std::string result = type_name.str();
  for (const auto &ptr_or_array : pointer_array_vector) {
    result += ptr_or_array;
  }
  return result;
}
...
#ifdef ROSE_DEBUG
  if (i == clonePosition) {
    llvm::errs() << "clonePosition: ";
    i->print(llvm::errs());
    llvm::errs() << "\n";
    llvm::errs() << "Reading state variables from " << StateVariablesFilePath << "\n";
    std::vector<std::vector<std::string>> symbolic_vars = readStateVariables(StateVariablesFilePath);
    llvm::errs() << "Symbolic variables:\n";
    for (const auto &symbolic_var : symbolic_vars) {
      llvm::errs() << "Symbolic variable: ";
      for (const auto &var : symbolic_var) {
        llvm::errs() << var << " ";
      }
      llvm::errs() << "\n";
    }
    std::string default_value_save_path = "";
    std::ofstream default_value_file;
    if (ExtractDefaultValues) {
      llvm::SmallString<128> _path(StateVariablesFilePath);
      llvm::sys::path::remove_filename(_path);
      llvm::sys::path::append(_path, "TODO:");
      default_value_save_path = _path.str().str();
      default_value_file.open(default_value_save_path);
      if (!default_value_file) {
        llvm::errs() << "Failed to open default value file: " << default_value_save_path << "\n";
        exit(1);
      }
      llvm::errs() << "Extracting default values to: " << default_value_save_path << "\n";
    }
    bool symbolized_var_flag = !state.isSymbolized;
    std::vector<std::string> ignored_symbolic_vars = state.getDesymbolizedVars();
    if (ignored_symbolic_vars.size() == 1 && ignored_symbolic_vars[0] == "__ALL__") {
      llvm::errs() << "State " << state.getID() << " Skipping all symbolic variables.";
      symbolized_var_flag = false;
    }
    for (auto &symbolic_var : symbolic_vars) {
      if (!symbolized_var_flag) {
        continue;
      }
      bool skip = false;
      for (const auto &ignored_var : ignored_symbolic_vars) {
        if (symbolic_var.size() > 0 && symbolic_var[symbolic_var.size() - 1] == ignored_var) {
          skip = true;
          break;
        }
      }
      if (skip) {
        llvm::errs() << "State " << state.getID() << " Skipping symbolic variable: ";
        for (const auto &var : symbolic_var) {
          llvm::errs() << var << " ";
        }
        llvm::errs() << "\n";
      ...
      int index = 0;
      llvm::DIType *OriginalTy = nullptr;
      llvm::DIType *Ty = nullptr;
      assert(symbolic_var.size() > 0 && "symbolic_var should not be empty");
      llvm::errs() << "Processing symbolic variable: ";
      for (const auto &var : symbolic_var) {
        llvm::errs() << var << " ";
      }
      llvm::errs() << "\n";
      auto field = this->kmodule->module->getNamedGlobal(symbolic_var[index]);
      if (field == nullptr) {
        std::string possible_mangled_name = resolveManagedSymbol(symbolic_var[index]);
        llvm::errs() << "Looking for global variable : " << possible_mangled_name << "\n";
        field = this->kmodule->module->getNamedGlobal(possible_mangled_name);
      }
      uint64_t current_address = 0;
      if (field != nullptr) {
        llvm::errs() << "Found global variable: " << field->getName() << "\n";
        DIGlobalVariableExpression *DIGE = dyn_cast<DIGlobalVariableExpression>(field->getMetadata("dbg"));
        assert(DIGE != nullptr && "DIGE should not be null");
        Ty = DIGE->getVariable()->getType();
        OriginalTy = Ty;
        assert(Ty != nullptr && "Ty should not be null");
        const MemoryObject *mo = globalObjects[field];
        uint64_t address = mo->address;
        current_address = address;
        llvm::errs() << "Found global variable address: " << address << "\n";
      }
      else {
        assert(!ExtractDefaultValuesFunctionName.empty() && "ExtractDefaultValuesFunctionName should not be empty");
        current_address = getLocalAddress(state, ExtractDefaultValuesFunctionName, symbolic_var[index]);
        if (current_address == 0) {
          llvm::errs() << "Could not find local variable: " << symbolic_var[index] << "\n";
          continue;
        }
        Ty = getLocalDIType(state, ExtractDefaultValuesFunctionName, symbolic_var[index]);
        OriginalTy = Ty;
        if (!Ty) {
          current_address = getLocalAddress(state, "__klee_posix_wrapped_main", symbolic_var[index]);
          Ty = getLocalDIType(state, "__klee_posix_wrapped_main", symbolic_var[index]);
          OriginalTy = Ty;
        }
      }
      index++;
      ...
      size_t bit_size = 0;
      size_t byte_size = 0;
      size_t object_address = current_address;
      size_t object_offset = 0;
      bool ignore_symbolize = false;
      bool stop_flag = false;
      llvm::errs() << "Current address: " << current_address << "\n";
      if (auto *DT = dyn_cast<DIDerivedType>(Ty)) {
        llvm::errs() << "Derived Type: " << DT->getName() << "\n";
        while (DT = dyn_cast<DIDerivedType>(Ty)) {
          if (DT->getTag() == llvm::dwarf::DW_TAG_pointer_type) {
            llvm::errs() << "Pointer Type: " << DT->getName() << "\n";
            Ty = DT->getBaseType();
            if (current_address == 0) {
              stop_flag = true;
              llvm::errs() << "Current address is 0, stopping pointer dereference.\n";
              break;
            } else {
              object_address = current_address;
            }
            ref<Expr> read_data = executeMemoryRead(state, current_address, 8);
            current_address = cast<ConstantExpr>(read_data)->getZExtValue();
            llvm::errs() << "Data: " << read_data << "\n";
            llvm::errs() << "Current address: " << current_address << "\n";
            assert(Ty != nullptr && "Ty should not be null");
            llvm::errs() << "Base Type: " << Ty->getName() << "\n";
            pointer_depth++;
          } else if (DT->getTag() == llvm::dwarf::DW_TAG_typedef) {
            llvm::errs() << "Typedef Type: " << DT->getName() << "\n";
            Ty = DT->getBaseType();
            assert(Ty != nullptr && "Ty should not be null");
            llvm::errs() << "Base Type: " << Ty->getName() << "\n";
            uint64_t type_size = Ty->getSizeInBits() / 8;
            llvm::errs() << "Type Size: " << type_size << "\n";
          } else {
            break;
          }
        }
      ...
      assert(Ty != nullptr && "Ty should not be null");
      if (index == symbolic_var.size()) {
        auto type_name = Ty->getName();
        static const std::unordered_set<std::string> standardTypes = {
            "int", "char", "bool", "long", "short",
            "unsigned", "signed", "size_t", "uint64_t", "uint32_t", "uint16_t",
            "uint8_t", "int64_t", "int32_t", "int16_t", "int8_t"
        };
        if (type_name.empty() || pointer_depth > 0 || standardTypes.count(type_name.str()) == 0) {
          ignore_symbolize = true;
          llvm::errs() << "[!] Ignoring symbolization for type: " << type_name << " (pointer depth: " << pointer_depth << ")\n";
        }
        bit_size = Ty->getSizeInBits();
        byte_size = bit_size / 8;
        uint64_t data = 0;
        std::string data_string = "unknown";
        if (pointer_depth > 0) {
          data = current_address;
          data_string = std::to_string(data);
        } else {
          ref<Expr> read_data = executeMemoryRead(state, current_address, byte_size);
          if (byte_size <= 8) {
            data = cast<ConstantExpr>(read_data)->getZExtValue();
            data_string = std::to_string(data);
          } else {
            data = 0;
            llvm::errs() << "Long Const: ";
            cast<ConstantExpr>(read_data)->print(llvm::errs());
            llvm::errs() << "\n";
            cast<ConstantExpr>(read_data)->toString(data_string);
          }
        }
        llvm::errs() << "Data: " << data << "\n";
        if (ExtractDefaultValues) {
          assert(state.id == 1 && "state id should be 1 for default values");
          llvm::errs() << "Saving default value for: " << symbolic_var[index - 1] << "\n";
          std::string type_name = getDerivedTypeName(OriginalTy);
          default_value_file << symbolic_var[index - 1] << " (" << type_name << ") " << data_string << " " << pointer_depth << "\n";
        }
      }
      if (stop_flag) {
        llvm::errs() << "Current address is 0, skipping field search for symbolic variable: " << symbolic_var[index - 1] << "\n";
        continue;
      }
      unsigned MAX_DEPTH = 10;
      unsigned depth = 0;
      while (index < symbolic_var.size()) {
        depth++;
        if (depth > MAX_DEPTH) {
          llvm::errs() << "* Exceeded maximum depth of " << MAX_DEPTH
                       << " while processing type: " << Ty->getName() << "\n";
          exit(1);
        }
        if (auto *CT = dyn_cast<DICompositeType>(Ty)) {
          errs() << "Composite Type: " << CT->getName() << "\n";
          for (const auto &Field : CT->getElements()) {
            if (auto *FieldCT = dyn_cast<DIType>(Field)) {
              if (FieldCT->getName() == symbolic_var[index]) {
                Ty = FieldCT;
                size_t field_bit_size = Ty->getSizeInBits();
                size_t field_byte_size = field_bit_size / 8;
                bit_size = field_bit_size;
                byte_size = field_byte_size;
                llvm::errs() << "Found field: " << FieldCT->getName() << " with bit size: " << field_bit_size << "\n";
                size_t field_byte_offset = FieldCT->getOffsetInBits() / 8;
                object_address = current_address + field_byte_offset;
                object_offset = field_byte_offset;
                ref<Expr> read_data = executeMemoryRead(state, object_address, field_byte_size);
                uint64_t data = cast<ConstantExpr>(read_data)->getZExtValue();
                llvm::errs() << "Data: " << read_data << "\n";
                index++;
                if (index == symbolic_var.size() && ExtractDefaultValues) {
                  assert(state.id == 1 && "state id should be 1 for default values");
                  llvm::errs() << "Saving default value for: " << symbolic_var[index - 1] << "\n";
                  std::string type_name = getDerivedTypeName(FieldCT);
                  default_value_file << symbolic_var[index - 1] << " (" << type_name << ") " << data << "\n";
                }
                break;
              }
            }
          ...
          llvm::errs() << "Found Derived Type: " << DT->getName() << "\n";
          assert(false && "DIDerivedType should not be used here, it should be DICompositeType");
          errs() << "Derived Type: " << DT->getName() << "\n";
          while ((DT = dyn_cast<DIDerivedType>(Ty)) && (DT->getTag() == llvm::dwarf::DW_TAG_pointer_type)) {
            llvm::errs() << "Pointer Type: " << DT->getName() << "\n";
            Ty = DT->getBaseType();
            assert(Ty != nullptr && "Ty should not be null");
            llvm::errs() << "Base Type: " << Ty->getName() << "\n";
          }
          if (Ty->getName() == symbolic_var[index]) {
            llvm::errs() << "Found base type: " << Ty->getName() << "\n";
            index++;
          }
          else {
          ...
          llvm::errs() << "Found Type: " << Ty->getName() << "\n";
          assert(false && "Bad type, should be DICompositeType or DIDerivedType");
          errs() << "Type: " << Ty->getName() << "\n";
          index++;
          break;
        }
      }
      if (index == symbolic_var.size() && !ignore_symbolize) {
        assert(index > 0 && "index should be greater than 0");
        llvm::errs() << "Symbolizing variable: " << symbolic_var[index - 1] << "\n";
        assert(bit_size > 0 && "bit_size should be greater than 0");
        assert(byte_size > 0 && "byte_size should be greater than 0");
        assert(object_address > 0 && "object_address should be greater than 0");
        assert(object_offset >= 0 && "object_offset should be greater than or equal to 0");
        llvm::errs() << "*** bit_size: " << bit_size << ", byte_size: " << byte_size
                     << ", object_address: " << object_address
                     << ", object_offset: " << object_offset << "\n";
        MemoryObject *new_mo = memory->allocate(byte_size, false, true, &state, nullptr, 8);
        if (!new_mo) {
          llvm::errs() << "Failed to allocate memory for symbolic variable: " << symbolic_var[index - 1] << "\n";
          exit(1);
        }
        ObjectState *new_os = bindObjectInState(state, new_mo, false);
        std::vector<ref<Expr>> arguments;
        arguments.push_back(new_mo->getBaseExpr());
        ref<Expr> sizeExpr = ConstantExpr::create(byte_size, Expr::Int64);
        arguments.push_back(sizeExpr);
        ref<Expr> nameExpr = ConstantExpr::create(0, Expr::Int8);
        nameExpr->name = symbolic_var[index - 1];
        arguments.push_back(nameExpr);
        llvm::errs() << "Symbolizing newly allocated global variable for: " << symbolic_var[index - 1] << ", with arguments: ";
        for (int i = 0; i < arguments.size(); i++) {
          if (i == arguments.size() - 1) {
            llvm::errs() << arguments[i]->name << " ";
          } else {
            llvm::errs() << arguments[i] << " ";
          }
        }
        llvm::errs() << "\n";
        specialFunctionHandler->handleMakeSymbolic(state, nullptr, arguments);
        ref<Expr> write_addr = ConstantExpr::create(object_address, Expr::Int64);
        ObjectPair op;
        bool success, success_resolve;
        success_resolve = state.addressSpace.resolveOne(state, solver.get(), new_mo->getBaseExpr(), op, success);
        assert(success_resolve && "resolveOne failed");
        assert(success && "resolveOne failed");
        const MemoryObject *target_mo = op.first;
        const ObjectState *target_os = op.second;
        ref<Expr> value = target_os->read(0, bit_size);
        llvm::errs() << "Value read from symbolic variable: " << value << "\n";
        success_resolve = state.addressSpace.resolveOne(state, solver.get(), write_addr, op, success);
        assert(success_resolve && "resolveOne failed");
        assert(success && "resolveOne failed");
        target_mo = op.first;
        target_os = op.second;
        ObjectState *wos = state.addressSpace.getWriteable(target_mo, target_os);
        wos->write(object_offset, value);
        llvm::errs() << "Write symbolic variable to address: " << write_addr << "\n";
      }
    }
    state.isSymbolized = true;
    if (ExtractDefaultValues) {
      if (default_value_file.is_open()) {
        default_value_file.close();
      }
      llvm::errs() << "Extracted default values, exiting...\n";
      exit(0);
    }
  }
  std::set<std::uint32_t> interestingStateIDs;
  if (interestingStateIDs.find(state.getID()) != interestingStateIDs.end()) {
    llvm::errs() << "** interesting state id: " << state.getID() << "\n";
    llvm::errs() << "** Executing: ";
    i->print(llvm::errs());
    llvm::errs() << "\n";
  }
...
bool Executor::checkMemoryUsage() {
  ...
  #ifdef ROSE
  std::map<std::string, unsigned int> argCountMap;
  for (auto &state : states) {
    auto concreteArgs = state->getConcreteArgs();
    for (const auto &arg : concreteArgs) {
      if (arg.size() > 1 && arg[0] == '-') {
        if (argCountMap.find(arg) == argCountMap.end()) {
          argCountMap[arg] = 1;
        } else {
          argCountMap[arg]++;
        }
  ...
  for (unsigned i = 0, N = arr.size(); N && i < toKill; ++i, --N) {
    ...
    bool found = false;
    unsigned idx = 0;
    for (unsigned int t = 0; t < tryTimes; ++t) {
      idx = theRNG.getInt32() % N;
      if (arr[idx]->coveredNew)
        idx = theRNG.getInt32() % N;

      auto concreteArgs = arr[idx]->getConcreteArgs();
      bool rareFlag = false;
      for (const auto &arg : concreteArgs) {
        if (arg.size() > 1 && arg[0] == '-') {
          auto it = argCountMap.find(arg);
          assert(it != argCountMap.end() && "arg must be in argCountMap");
          if (it->second <= 1) {
            rareFlag = true;
            break;
          }
...
void Executor::run(ExecutionState &initialState) {
  ...
  #ifdef ROSE
  get_module_symbols(this, this->kmodule->module.get());
  if (ExtractDefaultValuesFilePath.empty() || ExtractDefaultValuesLineNo == 0) {
    llvm::errs() << "ExtractDefaultValuesFilePath or ExtractDefaultValuesLineNo is not set\n";
    exit(1);
  }

  bool found = false;
  for (Function &F : *this->kmodule->module) {
    llvm::errs() << "Function: " << F.getName() << "\n";
    if ((F.getName() != "__klee_posix_wrapped_main") &&
        (F.getName() != ExtractDefaultValuesFunctionName))
      continue;
    for (BasicBlock &BB : F) {
      for (Instruction &I : BB) {
        ...
        outs() << "DebugLoc: " << DL << "\n"
               << "DILocation: " << Loc->getFilename() << ":" << Line << "\n"
               << "DIScope: " << Scope->getFilename() << "\n"
               << "DIFile: " << File->getFilename() << "\n";
        I.print(outs());
        outs() << "\n";
        StringRef InputFilePath = ExtractDefaultValuesFilePath;
        unsigned InputLineNumber = ExtractDefaultValuesLineNo;
        StringRef InputFileBase = sys::path::filename(InputFilePath);
        StringRef CurrentFileBase = sys::path::filename(FileName);
        if (CurrentFileBase.equals(InputFileBase) && Line == InputLineNumber) {
          errs() << "Found target instruction:\n"
          << "  File: " << FileName << "\n"
          << "  Line: " << Line << "\n"
          << "  Function: " << F.getName() << "\n"
          << "  Instruction: " << I << "\n";
  ...
  assert(found && "Cannot find target instruction");
  assert(initialStatePtr == nullptr && "initialStatePtr must be null");
  initialStatePtr = initialState.clone();
  dumpState(initialStatePtr);
  #endif
  ...
  #ifdef ROSE
  llvm::errs() << "Reading initial sequences\n";
  std::vector<std::vector<std::string>> initialSequences = readInitialSequences(InitialSequencesFilePath);
  if (initialSequences.size() > 0 && ExtractDefaultValues) {
    llvm::errs() << "ExtractDefaultValues is set with initial sequences provided.";
    exit(1);
  }
  std::vector<ExecutionState*> initialNewStates = initialStatePtr->cloneWithNewSeqs(initialSequences);
  for (auto newState : initialNewStates) {
    updateArgs(newState, newState->getConcreteArgs());
    processTree->attach(initialState.ptreeNode, newState, &initialState, BranchType::NONE);
  }
  searcher->update(&initialState, initialNewStates, std::vector<ExecutionState *>());
  states.insert(initialNewStates.begin(), initialNewStates.end());
  #endif

  // main interpreter loop
  #ifdef ROSE
  int old_state_cnt = states.size();
  auto start_time = std::chrono::high_resolution_clock::now();
  const unsigned int FORK_PERIOD = 10; // seconds
  #endif
  while (!states.empty() && !haltExecution) {
    ExecutionState &state = searcher->selectState();
    #ifdef ROSE
    klee::encounterSymbol = false;
    klee::forkNewOptionStates = false;
    auto concreteArgs = state.getConcreteArgs();
    concreteOptionsOfCurrentState = state.getConcreteArgs();
    #endif
    KInstruction *ki = state.pc;
    stepInstruction(state);
    #ifdef ROSE
    try {
      ...
      llvm::errs() << "* Exception caught during executeInstruction of state id "
                   << state.getID() << ": " << e.what() << "\n";
      terminateStateEarly(state, "* Exception during instruction execution",
    ...
    auto elapsed_secs = std::chrono::duration_cast<std::chrono::seconds>(current_time - start_time).count();
    bool PRUNE_UNSAT_SUBTREE = true;
    if (elapsed_secs >= FORK_PERIOD) {
      start_time = current_time;
      std::vector<ExecutionState*> statesToBeForked = selectStatesToBeUpdated(processTree.get());
      for (auto stateToBeForked : statesToBeForked) {
        solveStateConstraintsWithOptionInfo(stateToBeForked);
        if (!klee::solverSat) {
          if (!PRUNE_UNSAT_SUBTREE) {
            removedStates.push_back(stateToBeForked);
            continue;
          }

          std::vector<PTreeNode*> path_to_root;
          PTreeNode* current_node = stateToBeForked->ptreeNode;
          while (current_node != nullptr) {
            path_to_root.push_back(current_node);
            current_node = current_node->parent;
          }
          int left = 0;
          int right = path_to_root.size() - 1;
          int unsat_ancestor_index = -1;
          while (left <= right) {
            int mid = left + (right - left) / 2;
            PTreeNode* mid_node = path_to_root[mid];
            if (mid_node->state != nullptr && !checkConstraintsWithOptionInfo(mid_node->state)) {
              unsat_ancestor_index = mid;
              left = mid + 1;
            } else {
              right = mid - 1;
            }
          }
          assert(unsat_ancestor_index != -1 && "There must be an UNSAT ancestor");
          PTreeNode* unsat_ancestor_node = path_to_root[unsat_ancestor_index];
          std::queue<PTreeNode *> q;
          q.push(unsat_ancestor_node);
          while (!q.empty()) {
            PTreeNode* node = q.front();
            q.pop();
            removedStates.push_back(node->state);
            #ifdef ROSE_OPTION_SEARCHER
            if (node->left.first) {
              q.push(node->left.first);
            }
    ...
    #endif
    #ifdef ROSE
    if (pendingNewSequences.size() > 0) {
      auto steppedInstructions = state.steppedInstructions;
      assert(steppedInstructions > 0 && "steppedInstructions must be > 0");
      std::vector<ExecutionState*> newStates = initialStatePtr->cloneWithNewSeqs(pendingNewSequences);
      pendingNewSequences.clear();
      #ifdef ROSE_DEBUG
      llvm::errs() << "old id: " << state.getID() << "\n";
      auto oldConcreteArgs = state.getConcreteArgs();
      llvm::errs() << "old concreteArgs.size() = " << oldConcreteArgs.size()
                  << "\n";
      for (auto arg : oldConcreteArgs) {
        llvm::errs() << arg << ", ";
      }
      llvm::errs() << "\n";
      #endif
      assert(klee::newOptionStates.size() == 0);
      for (auto newState : newStates) {
        #ifdef ROSE_DEBUG
        llvm::errs() << "new id: " << newState->getID() << "\n";
        auto newConcreteArgs = newState->getConcreteArgs();
        llvm::errs() << "new concreteArgs.size() = " << newConcreteArgs.size()
                    << "\n";
        for (auto arg : newConcreteArgs) {
          llvm::errs() << arg << ", ";
        }
        llvm::errs() << "\n";
        #endif
        newState->targetSteppedInstructions = steppedInstructions;
        updateArgs(newState, newState->getConcreteArgs());
        addedStates.push_back(newState);
        klee::newOptionStates.push_back(newState);
        processTree->attach(state.ptreeNode, newState, &state, BranchType::NONE);
      }
      klee::forkNewOptionStates = true;
    }
    #endif
    updateStates(&state);
    ...
    #ifdef ROSE
    int new_state_cnt = states.size();
    old_state_cnt = new_state_cnt;
    #endif
  }
  ...
  llvm::errs() << "Visited concrete args: \n";
  for (auto argVector : visitedConcreteArgs) {
...
#ifdef ROSE_DEBUG
  llvm::errs() << "terminateStateEarly state id: " << state.id << "; ";
  llvm::errs() << "Concrete args: ";
  for (auto arg : state.getConcreteArgs()) {
    llvm::errs() << arg << " ";
  }
  llvm::errs() << "\n";
#endif
...
void Executor::terminateStateOnError(ExecutionState &state,
                                     ...
                                     const char *suffix) {
#ifdef ROSE_DEBUG
  llvm::errs() << "terminateStateOnError state id: " << state.id << "; ";
  llvm::errs() << "Concrete args: ";
  for (auto arg : state.getConcreteArgs()) {
    llvm::errs() << arg << " ";
  }
...
void Executor::executeAlloc(ExecutionState &state, ref<Expr> size, bool isLocal,
  ...
  if (ConstantExpr *CE = dyn_cast<ConstantExpr>(size)) {
    ...
    if (!mo) {
      ...
      #ifdef ROSE
      if (isLocal){
        std::string var_name = target->inst->getName().str();
        if (!var_name.empty()) {
          uint64_t address = mo->address;
          StackFrame &sf = state.stack.back();
          sf.localAddresses[var_name] = address;
        }
      }
      #endif
...
#ifdef ROSE
void Executor::solveStateConstraintsWithOptionInfo(ExecutionState *state) {
  klee::updateOptions = true;
  Solver::Validity res;
  ref<Expr> true_condition = ConstantExpr::alloc(0, Expr::Bool);
  assert(coreSolverPtr != nullptr && "coreSolverPtr is null");
  bool success = coreSolverPtr->evaluate(Query(state->constraints, true_condition), res);
  klee::updateOptions = false;
}

bool Executor::checkConstraintsWithOptionInfo(ExecutionState *state) {
  assert(state != nullptr && "checkConstraintsWithOptionInfo: state is null");
  bool oldSolverSat = solverSat;
  solveStateConstraintsWithOptionInfo(state);
  bool result = solverSat;
  solverSat = oldSolverSat;
  return result;
}


ref<Expr> Executor::executeMemoryRead(ExecutionState &state, uint64_t address, uint64_t byte_size) {
  llvm::errs() << "executeMemoryRead address: " << address << " byte_size: " << byte_size << "\n";
  ref<Expr> addr = ConstantExpr::create(address, Expr::Int64);
  ObjectPair op;
  bool success, success_resolve = false;
  success_resolve = state.addressSpace.resolveOne(state, solver.get(), addr, op, success);
  assert(success_resolve && "resolveOne failed");
  assert(success && "resolveOne failed");
  ...
  if (byte_size == 1) {
    ...
    llvm::errs() << "Non-Standard Byte Size: " << byte_size << "\n";
    width = byte_size * 8;
  }
  ...
  if (isa<ConstantExpr>(result)) {
    ...
    llvm::errs() << "Non-Constant Read Result: " << result << "\n";
    assert(isa<ConstantExpr>(result) && "Read result is not a constant");
    return result;
  }
}
#endif
...
void Executor::runFunctionAsMain(Function *f, int argc, char **argv,
                                 char **envp) {
#ifdef ROSE
  assert(!startF && "startF must be null!");
  startF = f;
  g_argc = argc;
  g_argv = argv;
  g_envp = envp;
...
void Executor::dumpState(ExecutionState *es) {
  llvm::errs() << "(" << es << ",";
  llvm::errs() << " stack:[";
  auto next = es->stack.begin();
  ++next;
  for (auto sfIt = es->stack.begin(), sf_ie = es->stack.end(); sfIt != sf_ie;
       ++sfIt) {
    llvm::errs() << "('" << sfIt->kf->function->getName().str() << "',";
    if (next == es->stack.end()) {
      llvm::errs() << es->prevPC->info->line << "), ";
    } else {
      llvm::errs() << next->caller->info->line << "), ";
      ++next;
    }
  }
  llvm::errs() << "], ";
  ...
  llvm::errs() << "{";
  llvm::errs() << "'depth' : " << es->depth << ", ";
  llvm::errs() << "'queryCost' : " << es->queryMetaData.queryCost << ", ";
  llvm::errs() << "'coveredNew' : " << es->coveredNew << ", ";
  llvm::errs() << "'instsSinceCovNew' : " << es->instsSinceCovNew << ", ";
  llvm::errs() << "'md2u' : " << md2u << ", ";
  llvm::errs() << "'icnt' : " << icnt << ", ";
  llvm::errs() << "'CPicnt' : " << cpicnt << ", ";
  llvm::errs() << "}, ";
  llvm::errs() << "pc: " << es->pc->info->file << ":" << es->pc->info->line
               << ", ";
  llvm::errs() << "prevPC: " << es->prevPC->info->file << ":"
               << es->prevPC->info->line << ", ";
  llvm::errs() << "steppedInstructions: " << es->steppedInstructions << ", ";
  llvm::errs() << "ptreeNode: " << es->ptreeNode << ", ";
#ifdef ROSE
  llvm::errs() << ", concreteArgs: [";
  for (auto &arg : es->concreteArgs) {
    llvm::errs() << arg << ", ";
  }
  llvm::errs() << "], stateId: " << es->getID();
#endif
  llvm::errs() << ")\n";
}
