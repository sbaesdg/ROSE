...
#ifdef ROSE
bool klee::considerUpdateCurrent = true;
#endif
void WeightedRandomSearcher::update(ExecutionState *current,
                                    const std::vector<ExecutionState *> &addedStates,
                                    const std::vector<ExecutionState *> &removedStates) {
  #ifdef ROSE
  if (klee::considerUpdateCurrent) {
  #endif
  // update current
...
#define IS_OUR_NODE_VALID(n)                                                   \
  (((n).first != nullptr) && (((n).second & idBitMask) != 0))
#else
#define IS_OUR_NODE_VALID(n)                                                   \
...
ExecutionState &RandomPathSearcher::selectState() {
  ...
  while (!n->state) {
    if (!IS_OUR_NODE_VALID(n->left)) {
      ...
      --bits;
      n = ((flips & (1U << bits)) ? n->left : n->right).first;
    }
  }
  #else
  unsigned flips=0, bits=0;
...
void RandomPathSearcher::update(ExecutionState *current,
  ...
  for (auto es : addedStates) {
    ...
    #ifdef ROSE_OPTION_SEARCHER
    childPtr = parent ? ((parent->left.first == pnode) ? &parent->left
                                                              : &parent->right)
                      : &processTree.root;
    while (pnode && !IS_OUR_NODE_VALID(*childPtr)) {
      childPtr->second = (childPtr->second | idBitMask);
      pnode = parent;
      if (pnode)
        parent = pnode->parent;

      childPtr = parent
                     ? ((parent->left.first == pnode) ? &parent->left
                                                             : &parent->right)
                     : &processTree.root;
    }childPtr = parent ? ((parent->left.first == pnode) ? &parent->left
                                                              : &parent->right)
  ...
  for (auto es : removedStates) {
    ...
    while (pnode && !IS_OUR_NODE_VALID(pnode->left) &&
      ...
      auto childPtr =
          parent ? ((parent->left.first == pnode) ? &parent->left
                                                         : &parent->right)
                 : &processTree.root;
