...
private:
  ...
  void dumpState(ExecutionState *es);
  void dumpStates();
  void dumpPTree();

  #ifdef ROSE
  void updateArgs(ExecutionState *state, std::vector<std::string> concreteArgs);
  #endif
...
public:
  ...
  #ifdef ROSE
  std::vector<std::string> getConcreteArgs(const ExecutionState &state) override;
  std::map<const llvm::GlobalValue*, ref<ConstantExpr>> getGlobalAddresses() const;
  ref<Expr> executeMemoryRead(ExecutionState &state, uint64_t address, uint64_t byte_size);
  uint64_t getLocalAddress(ExecutionState &state, std::string function_name, std::string var_name);
  llvm::DIType* getLocalDIType(ExecutionState &state, std::string function_name, std::string var_name);
  void solveStateConstraintsWithOptionInfo(ExecutionState *state);
  bool checkConstraintsWithOptionInfo(ExecutionState *state);
  #endif
};
