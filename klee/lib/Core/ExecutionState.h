...
struct StackFrame {
  ...
  Cell *locals;
  #ifdef ROSE
  std::map<std::string, uint64_t> localAddresses;
  std::map<std::string, llvm::DIType*> localDITypes;
  #endif
...
public:
  ...
  std::vector<std::string> getConcreteArgs() const {
    return concreteArgs;
  }
  void setConcreteArgs(std::vector<std::string> args) {
    concreteArgs = args;
    return;
  }
  std::vector<std::string> getDesymbolizedVars() const {
    return desymbolizedVars;
  }
  void setDesymbolizedVars(std::vector<std::string> vars) {
    desymbolizedVars = vars;
    return;
  }
#endif
...
public:
  ...
  ExecutionState *cloneAdjustingArg(std::vector<std::string> oldOptions, std::string optionName);
  std::vector<ExecutionState*> cloneWithNewSeqs(std::vector<std::vector<std::string>> sequences);
  std::uint64_t targetSteppedInstructions = 0;
  #endif
