...
cl::list<Searcher::CoreSearchType> CoreSearch(
    ...
    cl::values(
        ...
        #ifdef ROSE
        ,
        clEnumValN(Searcher::ROSE, "rose", "use ROSE")
        #endif
        ),
    cl::cat(SearchCat));
...
Searcher *klee::constructUserSearcher(Executor &executor) {
  #ifdef ROSE
  bool roseEnabled = std::find(CoreSearch.begin(), CoreSearch.end(), Searcher::ROSE) != CoreSearch.end();
  if (roseEnabled){
    if (CoreSearch.size() > 1)
      klee_error("ROSE should be the only searcher enabled.");
    assert(CoreSearch.size() == 1);
    std::vector<Searcher *> defaultSearchers;
    for (auto type : {Searcher::RandomPath, Searcher::NURS_CovNew})
      defaultSearchers.push_back(getNewSearcher(type, executor.theRNG, *executor.processTree));
    auto searcher = new ROSESearcher(new InterleavedSearcher(defaultSearchers), executor.theRNG, *executor.processTree);
