...
size_t getTypeSize(llvm::Type *type) {
  if (type->isSized()) {
    assert(GlobalTheModule != nullptr);
    return GlobalTheModule->getDataLayout().getTypeAllocSize(type);
  }
  else {
    assert(false);
  }
...
ExecutionState::ExecutionState(const ExecutionState& state):
  ...
  #ifdef ROSE
  isSymbolized = state.isSymbolized;
  concreteArgs = state.concreteArgs;
  desymbolizedVars = state.desymbolizedVars;
  #endif
  for (const auto &cur_mergehandler: openMergeStack)
...
ExecutionState *ExecutionState::cloneAdjustingArg(std::vector<std::string> oldOptions, std::string optionName) {
  auto options = oldOptions;
  if (std::find(options.begin(), options.end(), optionName) == options.end()) {
    options.push_back(optionName);
  } else {
    options.erase(std::find(options.begin(), options.end(), optionName));
  }
  if (visitedConcreteArgs.find(options) != visitedConcreteArgs.end()) {
    return nullptr;
  }
  visitedConcreteArgs.insert(options);
  ExecutionState *newState = new ExecutionState(*this);
  newState->setID();
  newState->setConcreteArgs(options);
  return newState;
}
std::vector<ExecutionState*> ExecutionState::cloneWithNewSeqs(std::vector<std::vector<std::string>> sequences) {
  std::vector<ExecutionState*> newStates;
  for (auto &seq : sequences) {
    if (visitedConcreteArgs.find(seq) != visitedConcreteArgs.end()) {
      continue;
    }
    std::vector<std::string> concreteArgs;
    std::vector<std::string> desymbolizedVars;
    std::string DESYMBOLIZED_TOKEN = "__DESYMBOLIZE__";
    bool desymbolized = false;
    for (auto &s : seq) {
      if (s == DESYMBOLIZED_TOKEN) {
        desymbolized = true;
        continue;
      }
      if (desymbolized) {
        desymbolizedVars.push_back(s);
      } else {
        concreteArgs.push_back(s);
      }
    }
    #ifdef ROSE_DEBUG
    llvm::errs() << "Cloning with new concrete args: ";
    for (auto &s : concreteArgs) {
      llvm::errs() << s << " ";
    }
    llvm::errs() << "\n";
    llvm::errs() << "Cloning with new desymbolized args: ";
    for (auto &s : desymbolizedVars) {
      llvm::errs() << s << " ";
    }
    llvm::errs() << "\n";
    #endif
    visitedConcreteArgs.insert(seq);
    ExecutionState *newState = new ExecutionState(*this);
    newState->setID();
    newState->setConcreteArgs(concreteArgs);
    newState->setDesymbolizedVars(desymbolizedVars);
