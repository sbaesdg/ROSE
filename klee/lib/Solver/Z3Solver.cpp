...
#ifdef ROSE
  std::string tmp_path = "TODO:";
  if (klee::updateOptions) {
    std::string smt2_str(Z3_solver_to_string(builder->ctx, theSolver));
    size_t MAX_SMT2_SIZE = 100000;
    if (smt2_str.size() <= MAX_SMT2_SIZE) {
      std::ofstream smt2_file(tmp_path, std::ios::out | std::ios::trunc);
      if (!smt2_file) {
        klee_warning("Failed to open file %s for writing Z3 query", tmp_path.c_str());
        klee_warning("Z3 query:\n%s", smt2_str.c_str());
        exit(1);
      }
      smt2_file << smt2_str;
      smt2_file.flush();
      smt2_file.close();
      klee::globalOptionVarMap.send_constraint(tmp_path);
      return false;
    } else {
      klee::solverSat = true;
      return false;
