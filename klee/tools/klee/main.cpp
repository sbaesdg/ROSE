...
#ifdef ROSE
void addString(char **&strings, unsigned int &size, const char *newString) {
  char **newStrings = new char *[size + 1];

  for (unsigned int i = 0; i < size; ++i) {
    newStrings[i] = strings[i];
  }

  newStrings[size] = new char[strlen(newString) + 1];
  strcpy(newStrings[size], newString);
  newStrings[size][strlen(newString)] = '\0';

  strings = newStrings;
  ++size;
}

void addArgString(char **&strings, unsigned int &size, const char *newString) {
  assert(strings != nullptr && size >= 1);

  char **old = strings;
  char **ns  = new char *[size + 1];

  ns[0] = old[0];
  const size_t len = std::strlen(newString) + 1;
  ns[1] = new char[len];
  std::memcpy(ns[1], newString, len);
  ns[1][len - 1] = '\0';
...
#ifdef ROSE
#ifdef ROSE_DEBUG
      llvm::errs() << "before b.numArgs: " << b.numArgs << "\n";
      llvm::errs() << "b.args: ";
      for (unsigned int i = 0; i < b.numArgs; ++i) {
        llvm::errs() << b.args[i] << " ";
      }
      llvm::errs() << "\n";
#endif
      assert(b.numArgs > 0 && b.args && "b.args should have at least one element");
      auto concreteArgs = m_interpreter->getConcreteArgs(state);
      for (auto it = concreteArgs.rbegin(); it != concreteArgs.rend(); ++it) {
        addArgString(b.args, b.numArgs, it->c_str());
      }
#ifdef ROSE_DEBUG
      llvm::errs() << "after b.numArgs: " << b.numArgs << "\n";
      llvm::errs() << "b.args: ";
      for (unsigned int i = 0; i < b.numArgs; ++i) {
        llvm::errs() << b.args[i] << " ";
      }
